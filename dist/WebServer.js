...
const { swagger, router } = koagger_1.createAPI({ controllers: controller_1.default });
leanengine_1.init({ appId, appKey, masterKey });
const app = new koa_1.default()
    .use(koa_logger_1.default())
    .use(leanengine_1.koa2())
    .use(swagger())
    .use(koa_jwt_1.default({ secret: appKey, passthrough: true }));
routing_controllers_1.useKoaServer(app, Object.assign({ cors: { credentials: true }, authorizationChecker: ({ context: { state } }, required_roles) => {
        if (!state.user)
            return false;
